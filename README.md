# ¡Hola! <picture><img src="https://github.com/7oSkaaa/7oSkaaa/blob/main/Images/about_me.gif?raw=true" width="50px"></picture> Soy Jorge Gutiérrez

<picture>
  <img align="right" src="https://github.com/7oSkaaa/7oSkaaa/blob/main/Images/Right_Side.gif?raw=true" width="250px">
</picture>

Soy desarrollador Full Stack enfocado principalmente en el ecosistema **Java + Spring Boot** y tecnologías modernas de frontend como **React** y **Next.js**. Disfruto construir aplicaciones completas: desde el diseño de APIs y lógica backend hasta interfaces limpias, funcionales y enfocadas en la experiencia del usuario.

Me interesa especialmente el desarrollo backend, la arquitectura de software, autenticación, automatización de procesos y construcción de aplicaciones escalables. He trabajado en proyectos enfocados en usuarios reales, integrando autenticación empresarial, despliegue automatizado y flujos operativos completos.

✨ Me gusta aprender construyendo, enfrentar problemas reales y entender cómo funcionan las cosas más allá de solo hacer que “funcionen”.
---

# 🚀 Tecnologías

## 🧠 Backend
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=java&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=spring-boot&logoColor=white)
![Spring Security](https://img.shields.io/badge/Spring_Security-6DB33F?style=for-the-badge&logo=springsecurity&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-D82C20?style=for-the-badge&logo=redis&logoColor=white)

---

## 🎨 Frontend
![React](https://img.shields.io/badge/React.js-61DAFB?style=for-the-badge&logo=react&logoColor=black)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=next.js&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)
![TailwindCSS](https://img.shields.io/badge/TailwindCSS-38BDF8?style=for-the-badge&logo=tailwind-css&logoColor=white)
![TanStack Query](https://img.shields.io/badge/TanStack_Query-FF4154?style=for-the-badge&logo=reactquery&logoColor=white)

---

## ⚙️ Herramientas y DevOps
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=for-the-badge&logo=github-actions&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)

---

# 🛠️ Proyectos Destacados

## 🏋️ GimnasioUIS
Sistema institucional de reservas desarrollado para la Universidad Industrial de Santander, enfocado en gestión de cupos, asistencia, sanciones y autenticación empresarial mediante Microsoft Entra ID.

🛠️ **Java 21 + Spring Boot + PostgreSQL + Redis + Next.js**

🔐 OAuth2 + JWT • 🐳 Docker • ⚙️ CI/CD con GitHub Actions

🌐 https://gimnasiouis.com

🔗 [Ver repositorio](#)

---

## 🛍️ E-commerce de Zapatería + Dashboard
Aplicación fullstack con autenticación, gestión de productos, pedidos y panel administrativo.

🛠️ **React + Spring Boot + MySQL**

🔗 [Ver repositorio](https://github.com/JorgeGutierrez11/zapateria)

---

## 🗓️ Sistema de Gestión de Reservas
Aplicación de reservas con roles, autenticación y panel administrativo.

🛠️ **React + Spring Boot + PostgreSQL**

🔗 [Ver repositorio](https://github.com/JorgeGutierrez11/Hotel_Reservation)

---

# 🌱 Sobre mí

- 📚 Ingeniería de Sistemas — Universidad Industrial de Santander
- 👨‍💻 Interesado en backend, arquitectura de software y automatización
- 🌎 Español nativo | Inglés técnico para lectura de documentación
- 🚀 Actualmente aprendiendo más sobre escalabilidad, seguridad y despliegue de aplicaciones

---

# 📬 Contacto

<p align="center">
  <a href="mailto:jorge1706gutierrez@gmail.com">
    <img src="https://img.shields.io/badge/-Correo-EA4335?style=for-the-badge&logo=gmail&logoColor=white">
  </a>
</p>

---

<p align="center"><i>"No hay mejor forma de aprender que construyendo."</i></p>
